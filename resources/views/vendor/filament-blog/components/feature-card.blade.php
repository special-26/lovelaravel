@props(['post'])
<div class="grid sm:grid-cols-2 gap-y-5 gap-x-10 group">
    ...
    <div class="flex flex-col justify-center space-y-10 py-4 sm:pl-10">
        <div>
            <div class="mb-5">
                <a href="{{ route('filamentblog.post.show', ['post' => $post->slug]) }}" class="mb-4 block text-xl md:text-4xl font-semibold group-hover:underline">
                    {{ $post->title }}
                </a>
                <div>
        ...
        <div class="flex items-center gap-4">
            <img class="h-11 w-11 overflow-hidden rounded-full bg-zinc-300 object-cover md:object-fill text-[0]" src="{{ $post->user->avatar }}" alt="{{ $post->user->name() }}">
            <div>

@props(['post'])
<a class="group sm:flex rounded-xl hover:shadow-xl" href="{{ route('filamentblog.post.show', ['post' => $post->slug]) }}">
    <div class="flex-shrink-0 relative rounded-xl overflow-hidden h-[200px] sm:w-[250px] sm:h-[350px] w-full">
        <img class="size-full absolute top-0 start-0 object-cover group-hover:scale-110 transition duration-700 ease-in-out" src="{{ asset($post->featurePhoto) }}" alt="{{ $post->photo_alt_text }}" alt="Image Description">
    </div>
    <div class="grow">
